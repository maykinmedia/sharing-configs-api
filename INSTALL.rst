...
``src/sharing/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test sharing
...
* ``DB_NAME``: name of the database for the project. Defaults to ``sharing``.
* ``DB_USER``: username to connect to the database with. Defaults to ``sharing``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``sharing``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting sharing_db_1 ... done
       Starting sharing_web_1 ... done
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it sharing_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
...
.. _Github: https://github.com/maykinmedia/sharing/
...
environment variables. See ``src/sharing/conf/docker.py`` for
all settings.
