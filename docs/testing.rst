...
1. ``sharing-django``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``sharing-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

**develop** branch
------------------

1. ``sharing-django-develop``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``sharing-django-develop-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

pull requests
-------------
1. ``sharing-pr-django``: backend tests, runs ``./bin/jenkins_django_pr.sh``.
2. ``sharing-pr-js``: frontend tests, runs ``./bin/jenkins_js.sh``.
3. ``sharing-pr-sass``: checks that sass compiles, runs ``./bin/jenkins_sass.sh``.
4. ``sharing-pr-isort``: checks that imports are correctly
